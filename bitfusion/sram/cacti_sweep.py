...
if __name__ == "__main__":
    dir_path = "/home/liutielong/Files_2025/LLM_quantization/ANT-Quantization/ant_simulator/bitfusion/sram/"
    cache_sweep_data = CactiSweep(bin_file=os.path.join(dir_path, 'cacti/cacti'),
    ...
    eyeriss_read_energy = float(cache_sweep_data.get_data_clean(cfg_dict)['read_energy_nJ'])/eyeriss_line_size/8*1.e3
    eyeriss_write_energy = float(cache_sweep_data.get_data_clean(cfg_dict)['write_energy_nJ'])/eyeriss_line_size/8*1.e3
    ...
    line_size = 4 #bytes
    entries = total_size / (N*M*line_size)  # 逻辑上把总 SRAM 均分到 N×M 个单元
