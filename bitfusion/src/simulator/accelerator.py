...
class Accelerator(object):
    """
    __init__函数构建这个加速器的整体框架。
    get_mem_read_cycles/get_mem_write_cycles: 计算读写DRAM一定量的数据需要的cycle数。
    get_compute_stats: 计算计算指令需要的cycle数。
    get_compute_cycles: 计算一个卷积层的计算指令需要的cycle数, weight_stationary数据流。
    get_compute_cycles_output_stationary: 计算一个卷积层的计算指令需要的cycle数, output_stationary数据流。
    """
    ...
    def get_compute_stats(self, ic, oc, ow, oh, b, kw, kh, iprec, wprec, im2col=False, weight_stationary = False):
        ...
        args:
            ...
            im2col: boolean. If true, we assume the cpu does im2col. Otherwise,
                    we do convolutions channel-wise。如果为True, 表示支持把卷积摊平为矩阵乘法。
        """
        compute_stats = Stats()
    ...
    def get_compute_cycles(self, ic, oc, ow, oh, b, kw, kh, iprec, wprec, im2col=False):
        ...
        if im2col:
            # 相当于(oc, kw*kh*ic) x (kw*kh*ic, b*oh*ow)这样两个矩阵的乘法。
            ni = kw * kh * ic
            no = oc
            batch = b * oh * ow
