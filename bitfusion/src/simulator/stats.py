class Stats(object):
    ...
    抽象一次仿真统计的结果，统一记录总周期、
    内存等待、各命名空间的读写位数，并提供加法/乘法操作，方便累加不同循环或缩放为多次重复执行。
    """
    ...
    def __add__(self, other):
        # 对应字段逐项累加，返回新的 Stats；常用于把多层循环阶段叠加。
        ret = Stats()
    ...
    def get_energy_breakdown(self, energy_cost, dram_cost=6e-3):
        # 返回 [static_energy, dram_energy, sram_energy, core_energy]，用在 run_ant.py 的能耗归一化。
